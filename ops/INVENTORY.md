...
| 未決清單露出完整退回身分 | 本輪已做 | `ops/src/release/rollbackContract.js` `describeRollbackIdentityRecord`、未決清單 | 第 30 包；目前版與上一可用版列出 SHA／digest／靜態樹／schema；Compose／設定未記錄則標未記錄；bind-mount 不還原；schema 不相容不顯示退回按鈕 |
