...
```bash
# 列出 custom dump 內容（驗證可讀、非空）
docker exec 5151-postgres-A sh -c "pg_restore -l /backups/5151_shadow.dump | head"
# 校驗檔案 sha256
sha256sum backups/5151_shadow-*.dump
```
...
`restore.sh` 會：在目標建立 `5151_restore_test`、`pg_restore --exit-on-error`、
`psql -c "SELECT 'restore_ok';"` 驗證可連可查。**還原到獨立 database，不碰 `5151_shadow`。**
...
- 備份檔複製到第二台 NAS（異地）：`scp backups/... tori@$SYNOLOGY_HOST:/volume1/backups/5151/`。
