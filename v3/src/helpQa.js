...
export function defaultHelpQaItems() {
  return [
    ...
    },
    {
      ...
      answer: "同一間房常被重複刊登，標價或管理費不一定一樣。系統用規則交叉比對租金、額外月費、總月費、費用文字、坪數、樓層與來源，最多比三則，不是成交預測。主列表只留總月費最低的主卡；較貴的只活在展開列，並寫「先別打這則」，電話與 LINE 先收合。展開後會先給一句重點差異，並列出交叉比對不同的欄。若副卡其實不是同一間，登入後可按「不是同一間」：先只從你的列表拆開，不會立刻改大家看到的資料；多人這樣說，系統才會對全站拆開。一人亂點不會污染共享判斷。租金、服務費或其它費用有改，也會在列表註記變更時間。",
    },
    {
      id: "listing-fit",
