...
**目前線上**另有 `v3/`，埠 5153，資料 `data-v3/v3.db`。規劃見 `v3/ARCHITECTURE.md`。**之後新功能只做 v3**；v2 只維護、不再加功能。畫面上的產品名仍是「吉比租房物件追蹤」，頁尾寫 `ver. 3.47`。
...
- 目前版：`https://c5151.reversalplay.me` → `127.0.0.1:5153`，`https://jibbyrenth.reversalplay.me` → `127.0.0.1:5155`（同一台 v3），資料 `/DATA/AppData/591-tracker-v3`
...
3. Subdomain `jibbyrenth`，Domain `reversalplay.me` → Type `HTTP`，URL `http://127.0.0.1:5155`（與 c5151 同一條 tunnel、同一台 v3）
4. Subdomain `jibbyrentops`，Domain `reversalplay.me` → Type `HTTP`，URL `http://127.0.0.1:5154`（OPS，獨立容器）
...
Cloudflare Zero Trust 請為目前版加 Public Hostname：`c5151` → `http://127.0.0.1:5153`，`jibbyrenth` → `http://127.0.0.1:5155`。OPS 為 `jibbyrentops` → `http://127.0.0.1:5154`。同一條 tunnel，不要另開第二條。
...
- 目前版：`https://c5151.reversalplay.me` → `http://127.0.0.1:5153`，`https://jibbyrenth.reversalplay.me` → `http://127.0.0.1:5155`
