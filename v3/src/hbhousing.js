...
export function normalizeHbItem(item, { regionId, sectionId } = {}) {
  ...
  return {
    ...
    community_id: 0,
    community_name: String(item.community || item.communityName || item.buildName || item.commName || "").trim(),
    tags: JSON.stringify([...tags, item.community || item.communityName || item.buildName].filter((row) => String(row || "").trim())),
    refresh_time: "",
