...
- 旗標關閉時仍用必須有／希望有／不接受晶片；`rental_catalog_v2` 開啟後改成每列「未指定／要有／不要」，不要再畫三份相同條件
...
- 順序：h2 → 訪客提示 → 規則短連結 → 會員操作列 → 公開列表 → 表單（可隱藏）
- 訪客隱藏 `#demandForm` 與會員操作
- 公開卡片用 `.wish-card`（沿用 `.self-mine-card` 節奏），租金用 `.rent-price`
- 詳情可用頁內 overlay 或 `/w/:id`

行政區仍可多選。條件晶片必須 `flex-wrap`，375px 不得橫向裁切。不要放列表篩選列、希望縣市、最低月租、補充／常去地點、通勤分鐘、公開稱呼／電話／LINE。房租欄位標「房租上限」。步行捷運文案用「1 公里內」。表單文案用「格局／預計入住／租期／捷運／車站」。旗標關閉時條件由後台「許願房條件選單」維護；開啟後改讀共用條件目錄。
