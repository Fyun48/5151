...
| `--notify-new` / `--notify-relist` / `--notify-relist-bg` / `--notify-fee` / `--offline-sure` | 淺藍／淺綠／粉紅／紅 | **只給 webhook embed 與站內通知 dock**（全新／重刊／費用／確認已下架）。找房列表內文維持既有 tag token，不要套這組色 |
