...
export function defaultHelpQaItems() {
  return [
    ...
    },
    {
      ...
      answer: "可以，管理員在後台開通後，登入頁會出現社群按鈕。新帳號仍須完成信箱開通信。超過約兩個月沒登入，系統會暫停主動向外抓取與通知；你再登入就會自動恢復，不會另外寄信。一年未使用是否刪帳號仍在評估，尚未執行。",
    },
    {
      id: "notify-smtp",
    ...
    },
    {
      ...
      answer: "同一間房常被重複刊登，標價或管理費不一定一樣。系統用規則交叉比對租金、額外月費、總月費、費用文字、坪數、樓層與來源，不是成交預測。同屋源裡總月費最低的會當主卡；較貴或疑似同源的可點「同屋源」標籤向下展開，並可打開原站。若這則比同屋源貴，卡片會先寫「先別打這則」。租金、服務費或其它費用有改，也會在列表註記變更時間。確認同一間後，主列表只留較便宜的那則。",
    },
    {
      id: "listing-fit",
