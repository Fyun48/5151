...
| 目前正式版可記錄 DB 還原要求 | 本輪已做 | `ops/src/release/productionRelease.js` `describeDbRestoreOffer` `requestProductionDbRestore`、未決清單 | 第 28 包；已成功或已退回程式的目前正式版可記錄 DB 還原要求；必須確認字 `RESTORE-PRODUCTION-DB`；不自動還原、不改寫終態；未送出／執行中／狀態不明不走這扇門 |
