...
#### ✗✓ 再更正（連跑兩次量測後）：`ab06f0e` **確實是效能收益** ✓，先前的判斷是我**量測設計錯誤** ✗
- 把計數測試改成**連跑兩次**（同一 driver／同一 deps ✓）後實測：
  | 次數 | 查詢數 | 差異 |
  |---|---|---|
  | 第 1 次 | **19** | — |
  | 第 2 次 | **13** ✓ | **−6（約 −32%）** |
- 第二次完全消失的查詢 ✓：**`SELECT to_regclass($1) AS reg` × 5 全部不見** ✓✓
  （＝ `ab06f0e` 的共享表存在性快取真的省了 **5 筆／每個後續請求** ✓）
  ＋ **`SELECT DISTINCT search_key FROM listings`** 不見 ✓（＝ `2bfdcf8` 的 `pgSearchKeyMemo` ✓）。
- ⇒ 更正 ✓：`ab06f0e` **是效能收益** ✓；我上次只量「一次請求」就下「19 → 19 ⇒ 無收益」的結論 ✗
  —— **這是量測設計錯誤** ✗（單次請求內快取本來就不會有差 ✓）。**第 2 次因量測設計而誤判** ✓
  （第 1 次是「本地套件很慢被讀成卡住」✗）。
- ⇒ 教訓（已寫進流程 ✓）：**快取類改動必須量「穩態」✗ 不能量單次** ✓——
  單次量測對快取是**結構性無效** ✗。
- ⇒ 目前穩態 ✓：**13 筆／請求** ✓（距 astra 目標 ≤12 只差 **1 筆** ✓）。
