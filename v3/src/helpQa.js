  ...
  return [
    {
      ...
      answer: "會員不能開關這欄。要不要在列表顯示捷運，由管理員在後台「系統抓取底庫」統一開或關。開啟後，顯示與否看的是「人走得了的地圖路線」，不是房子到捷運站的直線距離。只有步行距離未滿 1.5 公里才會顯示站名與「步行約 xx 公里」。直線看起來很近、但要繞路走到 1.5 公里（含）以上，或還沒算出步行路線、沒有可信座標時，都不會顯示站名。捷運這一欄也不會再顯示騎車或分鐘。",
    },
    {
      id: "interval",
      question: "為什麼一般會員看不到、也不能改檢查間隔？",
      answer: "全站底庫由管理員在後台設定抓取節奏（預設約 15 分鐘，可改 1–120）。這是共用根底：各來源先爬進同一份資料庫。一般會員的列表與通知再依方案每 8 分鐘比對一次新來源；已贊助改成 5 分鐘。會員不能改底庫間隔，也不會每人各爬一遍。管理員不再用「每次抓取頁數」這個欄位；系統內部用固定頁數抓摘要。",
    },
    {
    ...
    },
    {
      ...
      answer: "不是。591 進階網址、來源開關、系統抓取底庫是管理員用的。抓取頁數已從畫面拿掉。確認已下架天數與底庫間隔會員可以看、不能改。你的比對節奏依方案是 8 或 5 分鐘。實際仍會依這些規則過濾與更新。",
    },
    {
      id: "notify-smtp",
