...
## 第二輪（同日）：identity sequence 的坑（live 測試抓到）

`v3/test/crawler-reads-parity.test.js` 的「PG 寫入後回讀」子測試第一次跑就紅：

> `duplicate key value violates unique constraint "data_revision_pkey"`

原因不是測試寫錯，而是**匯入工具的缺陷**：PostgreSQL 對「帶明確 id」的 INSERT **不會**推進
`GENERATED BY DEFAULT AS IDENTITY` 的 sequence，而匯入是照抄快照的 id（`data_revision` 在 fixture
裡有 3 列 → sequence 仍停在 1 → 應用第一次自動編號就撞 1）。

- 修法：`pgSchema.importStore()` 在 schema 鏡射與列匯入之後，對每張「`INTEGER PRIMARY KEY`（rowid
  alias）」的表跑 `setval(pg_get_serial_sequence('<schema>.<table>', 'id'), max(id)+1, false)`
  （`resyncIdentitySequences()`／`identityResyncSql()`），並在工具輸出印出處理過的表。
- 影響範圍：所有「INSERT 不帶 id」的表（`data_revision`、`feedback`、`crm_*`、`site_*`…）——如果漏了
  這步，切換後會員的寫入動作會在第一次自動編號時失敗。
- `v3/scripts/pg-import.mjs` 現在直接呼叫 `importStore()`（schema ＋ 匯入 ＋ sequence 一次做完），
  所以切換流程不會漏。單元測試：`pg-import-batches.test.js` 的
  「an imported store re-syncs the identity sequences」。
