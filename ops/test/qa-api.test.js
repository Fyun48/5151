...
function initGitRepo() {
  const dir = mkdtempSync(path.join(os.tmpdir(), "qa-api-repo-"));
  const remote = mkdtempSync(path.join(os.tmpdir(), "qa-api-remote-"));
...
async function seedCompletedQa(db) {
  ...
  await executeCodingTask(db, claimed, { provider: prov, repo, pr: makeStubPrGateway(), selfTest: async () => ({ ran: true, passed: true }), now: NOW });
  const { run } = createQaRun(db, { codingTaskId: task.id, repo, now: NOW });
  await executeQaRun(db, run, { repo, now: NOW });
  g.cleanup();
  return { iid, codingTaskId: task.id, qaRunId: run.id };
}
...
test("45. owner QA inspection API requires auth; returns current + checks", async () => {
  await withServer(async ({ base, db }) => {
    const { codingTaskId, qaRunId } = await seedCompletedQa(db);
    assert.equal((await fetch(`${base}/ops/api/coding-tasks/${codingTaskId}/qa`)).status, 401);
    assert.equal((await fetch(`${base}/ops/api/qa-runs/${qaRunId}`)).status, 401);
    const { cookie } = await login(base);
    const view = await (await fetch(`${base}/ops/api/coding-tasks/${codingTaskId}/qa`, { headers: { cookie } })).json();
    assert.equal(view.coding_task_id, codingTaskId);
    assert.ok(view.current && view.current.final_result === "PASS");
    assert.ok(view.current.checks.length >= 10);
    const detail = await (await fetch(`${base}/ops/api/qa-runs/${qaRunId}`, { headers: { cookie } })).json();
    assert.equal(detail.id, qaRunId);
    assert.ok(Array.isArray(detail.checks));
  });
});

test("46. QA rerun mutation requires CSRF/Origin + owner; does not merge/deploy", async () => {
  await withServer(async ({ base, db }) => {
    const { codingTaskId } = await seedCompletedQa(db);
    assert.equal((await fetch(`${base}/ops/api/coding-tasks/${codingTaskId}/qa/rerun`, { method: "POST" })).status, 401);
    const { cookie, csrf } = await login(base);
    const noCsrf = await fetch(`${base}/ops/api/coding-tasks/${codingTaskId}/qa/rerun`, { method: "POST", headers: { cookie, "Content-Type": "application/json" }, body: "{}" });
    assert.equal(noCsrf.status, 403);
    const ok = await fetch(`${base}/ops/api/coding-tasks/${codingTaskId}/qa/rerun`, { method: "POST", headers: { cookie, "Content-Type": "application/json", "X-CSRF-Token": csrf, Origin: base }, body: "{}" });
    assert.equal(ok.status, 200);
    const out = await ok.json();
    assert.equal(out.rerun_requested, true);
    // rerun 只把 run 重設為 pending，未 merge、未部署（無此類副作用）。
    const runRow = db.prepare("SELECT status FROM development_qa_run WHERE id=?").get(out.qa_run_id);
    assert.equal(runRow.status, "pending");
    assert.ok(db.prepare("SELECT COUNT(*) n FROM audit_log WHERE action='issue.qa.rerun_requested'").get().n >= 1);
  });
});

test("QA API exposes no raw PII/secrets", async () => {
  await withServer(async ({ base, db }) => {
    const { codingTaskId } = await seedCompletedQa(db);
    const { cookie } = await login(base);
...
test("unknown coding task / qa run returns 404 for owner", async () => {
  await withServer(async ({ base }) => {
    const { cookie } = await login(base);
    assert.equal((await fetch(`${base}/ops/api/coding-tasks/99999/qa`, { headers: { cookie } })).status, 404);
    assert.equal((await fetch(`${base}/ops/api/qa-runs/99999`, { headers: { cookie } })).status, 404);
  });
});
