...
| 未送出的分析／評估／提案可取消 | 已做 | `ops/src/feedbackAnalysis.js` `cancelAnalysis`、`ops/src/evaluation.js` `cancelEvaluation`、`ops/src/proposal.js` `cancelProposal`、未決清單 | 第 22 包；Owner 可取消 pending／failed_retry／processing；已完成的不改寫；晚到完成不寫入 current |
| 未送出的發布通知可取消 | 已做 | `ops/src/releaseCandidate.js` `cancelReleaseNotification`、未決清單 | 第 23 包；Owner 可取消 pending 通知；已送出的不宣稱撤回；晚到 retry 不外送、不寫回 sent |
| 未決清單可取消製作／QA／staging | 已做 | `ops/src/codingTask.js` `cancelCodingTask`、`ops/src/qaRun.js` `cancelQaRun`、`ops/src/stagingDeploy.js` `cancelStagingDeployment`、未決清單 | 第 24 包；未決清單可取消尚未完成的製作／QA／staging；已完成的 staging 不改寫；晚到完成不寫入 current |
| 未送出的正式發布可取消 | 已做 | `ops/src/release/productionRelease.js` `cancelProductionReleaseRun`、未決清單 | 第 25 包；未送出且尚未受理 workflow 的正式發布可取消；已受理／已成功／狀態不明不改寫；晚到 execute 不外送 |
| 已受理正式發布觀察／取消 runner | 已做 | `ops/src/release/productionRelease.js` `observeProductionReleaseProgress` `cancelProductionReleaseRunner`、未決清單 | 第 26 包；已受理且尚未結束的 GitHub runner 可取消；不改寫正式終態、不宣稱撤回部署；已結束／狀態不明／未受理不走這扇門；晚到 execute 不外送、不寫 SUCCEEDED |
| 目前正式版可程式退回 | 已做 | `ops/src/release/productionRelease.js` `describeCodeRollbackOffer` `requestCodeRollback`、未決清單 | 第 27 包；已成功且為目前正式版的發布可程式退回上一版；必須回傳上一版身分；不猜 latest；不執行 DB 還原；未送出／執行中／狀態不明不走這扇門 |
| 目前正式版可記錄 DB 還原要求 | 已做 | `ops/src/release/productionRelease.js` `describeDbRestoreOffer` `requestProductionDbRestore`、未決清單 | 第 28 包；已成功或已退回程式的目前正式版可記錄 DB 還原要求；必須確認字 `RESTORE-PRODUCTION-DB`；不自動還原、不改寫終態；未送出／執行中／狀態不明不走這扇門 |
...
| 未決清單可按 Owner 手動重評／解除封鎖 | 已做 | `ops/src/reevaluation.js` `describeOwnerReevalOffer` `describeOwnerUnblockOffer`、未決清單 | 第 33 包；自動重評已停的暫緩／拒絕可手動重評；已封鎖可解除；只重開評估不開 PR、不部署；必須寫原因；已重開不再列出；Owner 直達不經此門 |
| 未決清單可確認正式發布實際結果 | 已做 | `ops/src/release/productionRelease.js` `describeUnknownProductionOffer` `confirmUnknownProductionResult`、未決清單 | 第 34 包；狀態不明可確認已成功／已失敗／已退回；只寫觀察不改寫終態、不宣稱撤回；必須寫原因與 observed_result；確認後不再擋移交；Owner 直達不經此門 |
| 未決清單可確認已送出遠端客服套用結果 | 已做 | `ops/src/siteCommand.js` `describeSiteCommandApplyOffer` `confirmSiteCommandApplyObservation`、未決清單 | 第 35 包；已送出且尚未套用成功可確認已套用／未套用／套用不明；只寫觀察不改寫終態、不假裝本站已回覆；必須寫原因與 observed_apply；確認後不再列出；Owner 直達不經此門 |
| 未決清單可確認本站停止遞送實際結果 | 已做 | `ops/src/siteDelivery.js` `describeSiteDeliveryOffer` `confirmSiteDeliveryObservation`、未決清單 | 第 36 包；解除訂閱或退出中可確認已停送／仍在送／停送不明；只寫觀察不改寫訂閱終態、不假裝本站已停送；必須寫原因與 observed_delivery；確認後不再列出；Owner 直達不經此門 |
| 未決清單可重試已阻擋的退出紀錄 | 本輪已做 | `ops/src/exitRetry.js` `describeExitRetryOffer` `retryBlockedExit`、未決清單 | 第 37 包；解除訂閱被未決工作擋住時可重試；只重拍未決快照，阻擋解除才標完成；不改寫訂閱終態、不假裝未決已消失；必須寫原因；已完成則冪等；Owner 直達不經此門 |
