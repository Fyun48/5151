...
`AUTH_EMAIL`／`AUTH_PASSWORD` 仍可當管理員種子帳號。新會員從登入頁註冊，須勾選免責聲明。一般會員每個設定檔最多 10 個行政區（可跨縣市）、每個帳號最多 3 個設定檔；管理員不受此限制。通知會寫「可使用 n 坪」，結尾房屋類型為「套房」、「電梯公寓/大樓」或「公寓」，不再寫「整層住家」。清除共用資料只有管理員能做。v1 的 `DATA_EPOCH` 沒有改。
