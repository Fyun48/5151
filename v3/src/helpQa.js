...
export function defaultHelpQaItems() {
  return [
    ...
    },
    {
      ...
      answer: "開通連結只能用一次。第一次成功後會登入並另寄歡迎信。再點同一封信會立刻顯示「已經使用過」或「找不到」，不會再轉進找房頁空等。",
    },
    {
      id: "oauth-idle",
      question: "可以用 Google、LINE、Facebook 註冊嗎？兩個月沒登入會怎樣？",
      answer: "可以，管理員在後台開通後，登入頁會出現社群按鈕。新帳號仍須完成信箱開通信。超過約兩個月沒打開本站（沒有已登入活動），系統會暫停主動向外抓取與通知；你再進來就會自動恢復，不會另外寄信。平常有開站使用不會因為沒重打密碼就被暫停。一年未使用是否刪帳號仍在評估，尚未執行。",
    },
    {
      id: "notify-smtp",
    ...
    },
    {
      ...
      answer: "要先到信箱點確認連結才算註冊成功，點過會登入並另寄歡迎信，這個連結只能用一次。再點同一封信會立刻顯示已使用或找不到，不會一直轉圈。3 天內沒點，連結會失效，系統會再寄一封失效通知；之後請用同一個信箱重新註冊，會再寄新的確認信。",
    },
  ];
}
