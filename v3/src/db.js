...
const changeLogSchemaReady = new WeakSet();

// PG 的變更紀錄表準備：鏡射建表 → 對齊 identity 序號（與 budgetGuardAsync／listingEnrichQueueAsync 同一慣例）。
//
// 為什麼不能省：PG 的 store 是「帶 id 匯入」的，PostgreSQL 不會因為帶 id 的 INSERT 推進
// GENERATED BY DEFAULT AS IDENTITY 序號。2026-09-24 Production 實測：data_revision 表內
// max(id)=685489、序號卻停在 305 → 每一次 bumpRevision 都撞 data_revision_pkey。
async function ensureChangeLogStoreOnce(pgDriver) {
  if (!pgDriver || changeLogSchemaReady.has(pgDriver)) return;
  ensureDataRevisionTable(db);
  await ensurePgSchema(pgDriver, db, { tables: ["data_revision"] });
  await resyncIdentitySequences(pgDriver, db, { tables: ["data_revision"] });
