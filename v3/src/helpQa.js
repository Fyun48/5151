...
export function defaultHelpQaItems() {
  return [
    ...
    },
    {
      ...
      answer: "會，但預設仍以 591 為主。管理員可另外打開住商、信義、5168、租租通、好房網：系統只抓該區租屋摘要，點進去是原站頁面，不是完整鏡像。樂屋被 Cloudflare 擋住，打開也不會抓。跨站若像同一間，會標成需確認同屋源，點標籤可展開雙方來源與費用差異，不會自動刪掉。列表可依「較適合」排序。這是免費找房工具，不是仲介、不經手金錢。",
    },
    {
      id: "same-house-cost",
      question: "同屋源為什麼有的比較貴？費用變更會標出來嗎？",
      answer: "同一間房常被重複刊登，標價或管理費不一定一樣。系統用規則交叉比對租金、額外月費、總月費、費用文字、坪數、樓層與來源，不是成交預測。同屋源裡總月費最低的會當主卡；較貴或疑似同源的標籤是按鈕，點下去展開簡易列表，可打開原站，並看到交叉比對差異。較貴那則會自動展開、寫「先別打這則」，電話與 LINE 先收合，避免手滑打到較貴刊登。租金、服務費或其它費用有改，也會在列表註記變更時間。確認同一間後，主列表只留較便宜的那則。",
    },
    {
      id: "listing-fit",
