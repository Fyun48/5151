...
function initGitRepo() {
  const dir = mkdtempSync(path.join(os.tmpdir(), "m14-api-repo-"));
  const remote = mkdtempSync(path.join(os.tmpdir(), "m14-api-remote-"));
...
async function withServer(run) {
  ...
  const repoRef = {};
  const seeded = await seedRC(db, repoRef);
...
test("migration-safety read API requires owner auth and is additive on release detail", async () => {
  await withServer(async ({ base, seeded }) => {
    assert.equal((await fetch(`${base}/ops/api/coding-tasks/${seeded.codingTaskId}/migration-safety`)).status, 401);
    const { cookie } = await login(base);
    const view = await (await fetch(`${base}/ops/api/coding-tasks/${seeded.codingTaskId}/migration-safety`, { headers: { cookie } })).json();
    assert.equal(view.coding_task_id, seeded.codingTaskId);
    assert.equal(view.assessment, null);
    const release = await (await fetch(`${base}/ops/api/coding-tasks/${seeded.codingTaskId}/release`, { headers: { cookie } })).json();
    assert.ok(release.migration_safety);
    assert.equal(release.migration_safety.assessment, null);
    assert.doesNotMatch(JSON.stringify(release), /leak@example\.com|reporter-\d/);
  });
});

test("migration-safety write API requires owner + CSRF/Origin and binds exact Gate #2 authorization", async () => {
  await withServer(async ({ base, db, seeded }) => {
    const m = seeded.manifest;
    const approveBody = JSON.stringify({ action: "APPROVE_RELEASE", manifest_id: m.id, manifest_version: m.manifest_version, manifest_hash: m.manifest_hash, artifact_digest: m.artifact_digest, head_sha: m.head_sha });
    const { cookie, csrf } = await login(base);
    const approved = await fetch(`${base}/ops/api/coding-tasks/${seeded.codingTaskId}/release/decision`, {
      method: "POST", headers: { cookie, "Content-Type": "application/json", "X-CSRF-Token": csrf, Origin: base }, body: approveBody,
    });
    assert.equal(approved.status, 200);
    const approvedJson = await approved.json();
    assert.ok(approvedJson.authorization.id > 0);
    assert.ok(approvedJson.migration_safety?.assessment?.clearance_result === "CLEARED_NO_MIGRATION");

    const assessBody = JSON.stringify({
      release_authorization_id: approvedJson.authorization.id,
      manifest_id: m.id,
    ...
    assert.equal((await fetch(`${base}/ops/api/coding-tasks/${seeded.codingTaskId}/migration-safety`, { method: "POST" })).status, 401);
    const noCsrf = await fetch(`${base}/ops/api/coding-tasks/${seeded.codingTaskId}/migration-safety`, {
      method: "POST", headers: { cookie, "Content-Type": "application/json" }, body: assessBody,
    });
    assert.equal(noCsrf.status, 403);
    const ok = await fetch(`${base}/ops/api/coding-tasks/${seeded.codingTaskId}/migration-safety`, {
      method: "POST", headers: { cookie, "Content-Type": "application/json", "X-CSRF-Token": csrf, Origin: base }, body: assessBody,
    });
    assert.equal(ok.status, 200);
    const out = await ok.json();
    assert.equal(out.idempotent, true);
    assert.equal(out.assessment.clearance_result, "CLEARED_NO_MIGRATION");
    const mismatch = await fetch(`${base}/ops/api/coding-tasks/${seeded.codingTaskId}/migration-safety`, {
      method: "POST", headers: { cookie, "Content-Type": "application/json", "X-CSRF-Token": csrf, Origin: base },
      body: JSON.stringify({ ...JSON.parse(assessBody), manifest_hash: "wrong" }),
    });
    assert.equal(mismatch.status, 409);
    assert.equal(db.prepare("SELECT COUNT(*) n FROM production_migration_safety_assessment").get().n, 1);
    const read = await (await fetch(`${base}/ops/api/coding-tasks/${seeded.codingTaskId}/migration-safety`, { headers: { cookie } })).json();
    assert.equal(read.fresh, true);
    assert.doesNotMatch(JSON.stringify(read), /leak@example\.com|AUTH_PASSWORD|ssh /i);
  });
});

test("unknown coding task migration-safety is 404 for owner", async () => {
  await withServer(async ({ base }) => {
    const { cookie } = await login(base);
    assert.equal((await fetch(`${base}/ops/api/coding-tasks/99999/migration-safety`, { headers: { cookie } })).status, 404);
  });
});
